...
router.post('/findPlace',async (req,res)=>{
    try {
        if(req.body.name) {
            var places = await place.find({ "name" : { $regex: req.body.name, $options: 'i' } })
        } else {
            var places = await place.find({ "category" : { $regex: req.body.category, $options: 'i' } })
        }
        res.send(places)
    }catch(err) {
        res.send(err)
...
router.delete('/:id', async (req, res) =>{
    try{
        ...
        if(!deletePlace) throw new Error('place deletion not done !');
